...
- **Known follow-ups (out of scope here):**
  - ~~`scripts/publish-videos.mjs`~~ **DONE (commit `26cdad0`).** It now writes URLs into the worktree's `data/data.sql` (via `scripts/lib/apply-video-urls.mjs`) and re-runs the exporter, so a published URL is durable and can't be wiped by the next `db:export`. Retired the dead `patch-catalog` helper. Note: because the `video-drops` worktree bases on `origin/master`, publishing requires the exercise DB to already be on master (a clear error fires otherwise) — i.e. merge this branch first.
