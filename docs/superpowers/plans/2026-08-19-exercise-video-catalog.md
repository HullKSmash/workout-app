...
- **Post-implementation correction (final review):** Task 2's generator originally derived the alternating flag from the exercise *name* (`deriveCore(ex.name).side`). Task 3 later moved side onto `ex.side` and made names side-free, so re-running the generator post-migration silently dropped `videoAlternating` from the 10 alternating movements — breaking the documented "regenerate after import" workflow (the committed catalog was fine because it was generated pre-migration). Fixed by extracting the required-map computation into `scripts/lib/collect-required.mjs` (reads `ex.side`) with a regression test asserting the 10 alternating slugs and idempotent regeneration. The generator code shown in Task 2 above predates this fix.
